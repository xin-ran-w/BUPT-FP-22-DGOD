...
def vis_stitch_img(img, trg, class_dict_path):

    trg = {k: v.numpy() for k, v in trg.items()}


    assert os.path.exists(class_dict_path), "json file {} dose not exist.".format(hparams.cls)
    json_file = open(class_dict_path, 'r')
    class_dict = json.load(json_file)
    json_file.close()
    category_index = {v: k for k, v in class_dict.items()}

    draw_box(image=img,
             scores=[1 for box in trg['boxes']],
             classes=trg['labels'],
